...
| 6 Compensation                | Done (full months) / gated (partial months) | base pay by service-month ordinal + confirmed prior-service credit; meal/transport from user-entered rates × derived eligible days; total only when all components calculate; persisted snapshots. Call-up/discharge-month proration and non-payable-day deductions stay gated (rounding not fixed by primary text). See `docs/COMPENSATION_POLICY_2026.md` |
| 7 Local-first storage         | Done (MVP)                                  | versioned document, migrations, recovery/quarantine, cross-tab rebase, offline shell. Offline edits sync later via the debounced/backoff sync scheduler                                                                                                                                                                                                     |
| 8 Cloud backup & sync         | Done (optional)                             | Supabase email-code sign-in, record sync on the causal merge contract, conflict UI, immutable cloud backups, cloud-data deletion with stale-device generation. See `docs/CLOUD_SYNC.md`. Missing: Apple/Google sign-in, tombstone GC, E2EE                                                                                                                  |
| 9 Export & privacy            | Partial                                     | JSON backup + validated merge/replace restore, events CSV, leave CSV, full local wipe. Cloud-data deletion done. Missing: compensation CSV                                                                                                                                                                                                                  |
