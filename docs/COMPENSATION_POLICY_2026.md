...
| Eligible days           | NOT_FOUND (in the MMA standard)         | Unchanged. The count uses the service period, the confirmed weekdays, holidays confirmed by the user each month, and the event timeline. A full-day leave is not attended. Half-day and minute leave, outing, late arrival, early leave, education and training need a decision from the user. Night rotation and residential service are UNSUPPORTED. |
