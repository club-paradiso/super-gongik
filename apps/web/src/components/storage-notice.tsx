...
export function StorageNotice({
  ...
  async function downloadQuarantine(key: string) {
    const raw = await store.readRaw(key);
    if (raw) {
      downloadTextFile(
  ...
  if (notice?.kind === "MIGRATED") {
    ...
    body = [
      "마지막 몇 개의 변경이 빠졌을 수 있어요. 읽지 못한 원본은 내려받아 보관할 수 있어요.",
    ];
    quarantineKey = notice.quarantineKey;
  } else if (notice?.kind === "CORRUPT") {
    title = "저장된 데이터를 읽지 못했어요.";
    body = [
      "원본은 지우지 않고 기기에 따로 보관했어요. 백업 파일이 있다면 복원하고, 없다면 원본을 내려받아 문의해 주세요.",
    ];
    quarantineKey = notice.quarantineKey;
      ...
      <div>
        ...
        {quarantineKey ? (
          <button
            ...
            onClick={() => void downloadQuarantine(quarantineKey)}
            type="button"
          >
            읽지 못한 원본 내려받기
          </button>
